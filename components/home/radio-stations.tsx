...
export default function RadioStations({ stations }: { stations: RadioStation[] }) {
  ...
  return stations.map((station) => (
    ...
    >
      <div className="relative w-full aspect-square overflow-hidden bg-[var(--surface-3)] mb-2.5 shadow-lg ring-1 ring-white/5 group-hover:ring-[var(--primary)]/40 transition-all duration-400 rounded-[4px]">
        {station.coverUrl ? (
          <Image
          ...
          <div className="w-full h-full bg-gradient-to-br from-[var(--surface-3)] to-[var(--surface-2)] flex items-center justify-center">
            <Radio size={32} weight="bold" className="text-[var(--muted)]/40" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <div className="absolute top-2 left-2 px-2 py-0.5 rounded-[2px] bg-[var(--primary)]/90 text-[9px] font-bold text-black uppercase tracking-wider">
          Radio
        </div>
      </div>
